...
export function Dashboard({ birthdays, user, onUpdateBirthday, onDeleteBirthday }: {
  ...
  // Prochain anniversaire (utilisé pour la Hero Card)
  const nextBirthday = upcoming[0] ?? null;

  return (
    <div className="p-6 space-y-8">

      {/* ── HERO CARD — Prochain anniversaire ───────────────── */}
      <motion.section
      ...
      >
        {nextBirthday ? (
          <motion.div
            ...
            style={{
              background: nextBirthday.daysUntil <= 7
                ? 'linear-gradient(135deg, #FF4B4B 0%, #cc2e2e 100%)'
                : 'linear-gradient(135deg, #1f2937 0%, #374151 100%)',
              boxShadow: nextBirthday.daysUntil <= 7
                ? '0 6px 0 #9b1c1c, 0 12px 40px rgba(255,75,75,0.3)'
                : '0 6px 0 #111827, 0 12px 40px rgba(0,0,0,0.2)',
            }}
          >
            ...
            <div className="relative z-10 p-5 flex items-center gap-4">
              ...
              <div className="relative shrink-0">
                <div className="w-16 h-16 rounded-2xl overflow-hidden border-2 border-white/30">
                  <img
                    src={nextBirthday.photoUrl || `https://picsum.photos/seed/${nextBirthday.id}/100/100`}
              ...
              <div className="flex-1 min-w-0">
                <p className="text-white/70 text-[11px] font-black uppercase tracking-widest mb-0.5">
              ...
              <div className="shrink-0 text-center">
                {nextBirthday.daysUntil === 0 ? (
                  <motion.div
                    ...
                    className="bg-white/20 rounded-2xl px-3 py-2"
                  >
                    <p className="text-white font-black text-xs leading-tight">Aujourd'hui</p>
                    <p className="text-2xl">🎂</p>
                  </motion.div>
                ) : nextBirthday.daysUntil === 1 ? (
                  <div className="bg-white/20 rounded-2xl px-3 py-2">
                    <p className="text-white/80 text-[10px] font-bold uppercase">Dans</p>
                    <p className="text-white font-black text-2xl leading-none">1</p>
                    <p className="text-white/80 text-[10px] font-bold uppercase">jour</p>
                  </div>
                ) : (
                  <div className="bg-white/15 rounded-2xl px-3 py-2">
                    <p className="text-white/70 text-[10px] font-bold uppercase">J-</p>
                    <p className="text-white font-black text-2xl leading-none">{nextBirthday.daysUntil}</p>
                    <p className="text-white/70 text-[10px] font-bold uppercase">jours</p>
                  </div>
                )}
              </div>
            </div>
          ...
          <div
            ...
            style={{ background: 'linear-gradient(135deg, #1f2937 0%, #374151 100%)', boxShadow: '0 6px 0 #111827' }}
          >
            <span className="text-4xl">🎂</span>
            <div>
              <p className="text-white font-black text-base">Aucun anniversaire enregistré</p>
              <p className="text-white/60 text-xs mt-1">Scanne un QR code pour ajouter ton premier ami !</p>
            </div>
          </div>
        )}
      </motion.section>
